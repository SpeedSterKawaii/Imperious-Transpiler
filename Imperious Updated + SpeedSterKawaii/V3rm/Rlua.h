...
using namespace Adresses; // Instead of Adresses:: we do this so we can include the namespace.

std::queue<std::string> ScriptQueue;

DWORD EmptySpace1 = Identity1; //Identity 1
DWORD EmptySpace2 = Identity1; //Identity 2

DWORD Add(DWORD func);
typedef int (*lRua_CFunction) (DWORD rL);
#define ASLR(x)(x - 0x400000 + (DWORD)GetModuleHandleA(0)) // ASLR Bypass.
...
using deser = int(luau_deserialize_CCV*)(DWORD v1, const char* v2, const char* v3, size_t v4);
deser r_lua_deserialize = (deser)unprotect(ASLR(luau_deserialize_addr));

using s_t = void(spawn_CCV*)(DWORD v1);
s_t r_lua_spawn = (s_t)unprotect(ASLR(spawn_addr));
...
sf_t r_lua_setfield = (sf_t)unprotect(ASLR(lua_setfield_addr));

using ps_t = void(lua_pushlstring_CCV*)(DWORD v1, const char* v2, int v3);
ps_t r_lua_pushlstring = (ps_t)unprotect(ASLR(lua_pushlstring_addr));
...
uintptr_t ReturnDataModel() // Run Get Data Model.
{
	DWORD DMPad[8];
	DWORD GetDataModelAdr = ASLR(getdatamodel_addr);// This is GDM.
	DWORD SomeRandomAdrIdkTheNameOf = ASLR(getdatamodel2_addr);// This is GDM 2.
...
static TValue* index2adr(lua_State* L, int idx) { // Index 2 address.
	if (idx > 0) {
